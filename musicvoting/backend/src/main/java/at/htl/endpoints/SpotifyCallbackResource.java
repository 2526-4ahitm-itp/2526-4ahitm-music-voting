...
public class SpotifyCallbackResource {
    ...
    public Response callback(@QueryParam("code") String code, @QueryParam("state") String state) {
        try {
            String partyIdStr = parsePartyIdFromState(state);
            if (partyIdStr == null) {
    ...
    public Response iosCallback(
        ...
        try {
            ...
            if (partyIdStr == null) {
                return Response.status(Response.Status.BAD_REQUEST)
                        .entity(Map.of("error", "Missing party ID in state"))
                        .type(MediaType.APPLICATION_JSON)
                        .build();
            }
    ...
    private String parsePartyIdFromState(String state) {
        if (state == null) return null;
        if (state.startsWith("web:")) {
            String[] parts = state.split(":", 2);
            return parts.length == 2 && !parts[1].isBlank() ? parts[1].trim() : null;
        }
        if (state.toLowerCase(Locale.ROOT).startsWith("ios:")) {
            String[] parts = state.split(":", 3);
            return parts.length == 3 && !parts[2].isBlank() ? parts[2].trim() : null;
        }
        return null;
    }
