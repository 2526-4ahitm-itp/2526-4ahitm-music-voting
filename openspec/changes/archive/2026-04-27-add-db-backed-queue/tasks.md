...
- [x] 1.3 Start docker compose stack and verify schema in IntelliJ ER diagram
...
- [x] 6.1 Confirm `GET /track/queue` returns correct sort order (likes DESC, added_at ASC) from DB
- [x] 6.2 Confirm adding the same song twice returns "Song ist schon in der Warteschlange."
- [x] 6.3 Confirm vote toggle: second vote from same deviceId removes the row and decreases count
- [x] 6.4 Confirm no Spotify playlist `GET` calls are made during normal queue reads (check logs)
- [x] 6.5 Confirm queue persists after backend restart
